...
def get_data_task(t):
    ...
    arr = []
    try:
        for i in config_for_readOF.ID_COLUMN.keys():
            try:
                data = getattr(t, i)
...
def fill_dataframe(project):
    ...
    if not config_for_readOF.ID_COLUMN:
    ...
    data = pd.DataFrame(columns=config_for_readOF.ID_COLUMN.values())
    try:
        for t in task_collection:
            data.loc[len(data.index)] = get_data_task(t)
    except Exception:
...
def set_style_excel(column_index, path_to_excel):
    ...
    """
    try:
        with open(config_for_readOF.PATH_TO_STYLE_FILE, 'rb') as file:
...
def main(path_to_project, path_to_folder):
    ...
    try:
        ...
        column_index = data.columns.get_loc(config_for_readOF.ID_COLUMN['Text5']) + 1
